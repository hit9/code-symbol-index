...
引用查询读取当前源码，只提取所查询的名称并跳过无关 AST 子树；schema 5 符号索引保持
不变，不写入引用、无需重建。名称摘要可让引用与调用图查询跳过未变化文件（见
[文件名称摘要](#文件名称摘要)）；大仓库仍有文件扫描与候选文件解析成本，详见
[性能实测报告](benchmarks/REPORT.md)。
...
进度输出：index/update 在被捕获或非 TTY 环境下不输出进度，减少 agent 输出。交互
...
引用和调用图查询利用紧凑的名称摘要跳过未变化的文件；可能命中的文件仍会扫描、
解析当前源码。下次写入时，schema 5 的 `files` 表会增加可空的 `name_summary` 字段，
读取旧库不做迁移；正常 `index` 会在不重建 AST 的前提下为未变化文件补齐摘要，
后续刷新只补齐缺失或 stat 失效的条目；指定路径的 `update` 只维护所选文件。
以少量存储和索引时间换取更少的查询读取。

以下情况沿用原扫描路径：前 32 次文件检查、没有摘要、特殊名称、超过 1 MiB 的
文件、最近一秒内变化的文件，以及 Windows（ctime 语义不同，详见
[os.stat_result.st_ctime](https://docs.python.org/3/library/os.html#os.stat_result.st_ctime)）。
仅当设备、inode、大小、mtime、ctime 均一致时才信任否定结果；检查只在单次请求内
复用，也不对并发编辑提供原子快照。单次请求的摘要内存有界，不保存全量引用或
源码上下文。
